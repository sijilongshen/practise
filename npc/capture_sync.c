 ...
 *
 * $Id: capture_sync.c 39498 2011-10-20 19:44:40Z tuexen $
 *
...
sync_pipe_input_cb(gint source, gpointer user_data)
{
    ...
    case SP_FILE:
        if(!capture_input_new_file(capture_opts, buffer)) {
            ...
            ws_close(source);
            g_print("Open capture file failed\n");
        ...
        g_print("Open capture file: %s\n",((capture_file *)capture_opts->cf)->filename);
        break;
...
g_print("SP_ERROR_MSG\n");
        break;
    case SP_BAD_FILTER: {
        char *ch;
        int index;
g_print("SP_BAD_FILTER\n");
        ch = strtok(buffer, ":");
    ...
    case SP_DROPS:
        dropped_packets = (guint32)strtoul(buffer, NULL, 10);
g_print("Drop %u packets\n",dropped_packets);
        capture_input_drops(capture_opts, dropped_packets);      
        break;
    default:
